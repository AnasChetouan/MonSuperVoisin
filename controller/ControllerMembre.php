   ...
   public static function readAllActions(){
    ...
    public static function connected(){
        if(!is_null(Dispatcher::myGet('mdp'))&& !is_null(Dispatcher::myGet('login'))){
            ...
            if(ModelMembre::checkPassword(htmlspecialchars(Dispatcher::myGet('login')),$mdp_chiffre)==true){//on vÃ©rifie si le couple login/mdp est prÃ©sent dans la bdd      
                $u = ModelMembre::select(Dispatcher::myGet('login'));
                if($u->getNonce()==0){
                    $_SESSION['login']=Dispatcher::myGet('login');        
                    if(ModelMembre::adminOrNot(Dispatcher::myGet('login'),$mdp_chiffre)==true){ //on vÃ©rifie si l'membre qui vient de se connecter est admin ou pas
                        $_SESSION['admin'] = true;
                    } else{
                    ...
                    $pb = "connexion";// $pb est une variable qui permet de gerer le retour en ariÃ¨rre (sur la view error.php)
                    require_once File::build_path(array("view","view.php"));
                }
        ...
        $mdp = htmlspecialchars(Dispatcher::myGet('mdp'));
            ...
            if(strlen(Dispatcher::myGet('mdp')) >= 6)
            {
                if(Dispatcher::myGet('mdp') == Dispatcher::myGet('mdp2')){
                    ...
                    //$u = new ModelMembre(Dispatcher::myGet('login'), $mdp_chiffre, Dispatcher::myGet('nom'), Dispatcher::myGet('prenom'), Dispatcher::myGet('email').'@yopmail.com',0,$nonce_aleatoire);// le 0 est la valeur de admin ce qui Ã©quivaut Ã  false en boolÃ©an
