  ...
  protected static $controller = "membre";
   ...
   public static function readAllActions(){
    ...
    public static function created() {
        $mdp = htmlspecialchars(Dispatcher::myGet('mdp'));
            ...
            if(strlen(Dispatcher::myGet('mdp')) >= 6)
            {
                ...
                $message = "Votre mot de passe est inférieur a 6 caractères";
                $view = "error";
                $pageTitle = "mdp<6";
                $pb = "mdp";
                $controller="membre";
    ...
    public static function updated() {
        ...
        $u = ModelMembre::select(Dispatcher::myGet('login'));
        if(Session::is_admin()){
            if(Dispatcher::myGet('admin')=="oui"){ //si le checkbox du formulaire de modification est coché alors l'membre visé devient admin ()
            // a voir s'il faut pas mettre l'écriture numérique de bool (1 pour true 0 pour false) car le type boolean (tinynumber) dans mysql est paramétré comme ça
                    $valeur = true; 
                }else{
                    $valeur = false;
            }
